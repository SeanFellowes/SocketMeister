        ...
        public static byte[] GenerateSendBytes(IMessage SendObject, bool Compress)
        {
            ...
            try
            {
                ...
                using (BinaryWriter bWriter = new BinaryWriter(stream))
                {
                    ...
                    //  WRITE HEADER
                    bWriter.Write(Convert.ToInt16(SendObject.MessageType));

                    if (Compress == false)
                    {
                        //  WRITE HEADER
                        bWriter.Write(false);    //  NOT COMPRESSED
                        bWriter.Write((int)0);   //  WILL BE USED TO WRITE _compressedLength
                        bWriter.Write((int)0);   //  WILL BE USED TO WRITE _uncompressedLength

                        //  WRITE USER BYTES
                        long position1 = stream.Position;
                        SendObject.AppendBytes(bWriter);
                        ...
                        stream.Position = 3;
                        bWriter.Write(messageLength);
                        bWriter.Write(messageLengthUncompressed);
                    }
                    else
                    {
                        //  WRITE HEADER
                        MemoryStream msUncompressed = null;
                        try
                        {
                            ...
                            using (BinaryWriter writer2 = new BinaryWriter(msUncompressed))
                            {
                                ...
                                if (writer2.BaseStream.Length > 1024)
                                {
                                    ...
                                    bWriter.Write(messageLength);
                                    bWriter.Write(messageLengthUncompressed);
                                    bWriter.Write(compressedBytes);

                                }
                                else
                                {
                                    bWriter.Write(false);     //  UNCOMPRESSED
                                    bWriter.Write((int)0);   //  WILL BE USED TO WRITE _compressedLength
                                    bWriter.Write((int)0);   //  WILL BE USED TO WRITE _uncompressedLength
                                    ...
                                    //  GO BACK AND FINISH HEADER
                                    stream.Position = 3;
                                    bWriter.Write(messageLength2);
                                    bWriter.Write(messageLengthUncompressed2);
                                }
                            }
                        }
                        finally
                        {
                            if (msUncompressed != null) msUncompressed.Dispose();
                        }
                    }
                }
